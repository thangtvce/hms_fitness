...
const StreakCelebrationPopup = ({ isVisible, streakCount, onClose }) => {
  ...
  const renderRays = () => {
    ...
    for (let i = 0; i < NUM_RAYS; i++) {
      ...
      // Tapered ray: use scaleX to make the base thinner and the end wider
      rays.push(
        <Animated.View
          key={`ray-${i}`}
          style={[
            styles.ray,
            {
              ...
              height: rayWidth,
              // scaleX: 1.7 makes the end much wider than the base
              transform: [
                { rotate: `${angle}deg` },
                { rotate: rayRotationDegrees },
                { scaleX: 1.7 }, // widen at the end
                { scaleY: 0.7 }, // keep ray not too thick
              ],
              // Make the left side (base) thinner using borderRadius
