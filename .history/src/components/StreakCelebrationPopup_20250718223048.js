...
const StreakCelebrationPopup = ({ isVisible, streakCount }) => {
    ...
    if (isVisible) {
      ...
      for (let i = 0; i < NUM_SPARKLES; i++) {
        ...
        sparklePropsRef.current.push({
          ...
          color: ["#f9bbdaff", "#FFB6C1", "#FFC0CB", "#FF1493", "#DA70D6"][Math.floor(Math.random() * 5)],
  ...
  const renderRays = () => {
    ...
    for (let i = 0; i < NUM_RAYS; i++) {
      ...
      // Màu sắc đậm ở tâm, nhạt dần ra ngoài
      const centerOpacity = 0.8 // Đậm ở tâm
      const middleOpacity = 0.4 // Trung bình ở giữa
      const outerOpacity = 0.05 // Rất nhạt ở ngoài
  ...
  <Defs>
    <SVGGradient id={`grad-${i}`} x1="0" y1="0" x2="0" y2="1">
      {/* Đậm ở tâm */}
      <Stop offset="0%" stopColor={`rgba(255,20,147,${centerOpacity})`} />
      <Stop offset="10%" stopColor={`rgba(255,20,147,${centerOpacity * 0.9})`} />
      <Stop offset="25%" stopColor={`rgba(255,105,180,${middleOpacity})`} />
      <Stop offset="50%" stopColor={`rgba(255,182,193,${middleOpacity * 0.4})`} />
      <Stop offset="75%" stopColor={`rgba(255,192,203,${outerOpacity * 0.2})`} />
      {/* Gần như trong suốt ở rìa ngoài */}
