...
import Svg, { Polygon, Defs, LinearGradient as SVGLinearGradient, Stop } from "react-native-svg" // Use LinearGradient from SVG
...
const StreakCelebrationPopup = ({ isVisible, streakCount, onClose }) => {
  ...
  // Define ray properties based on the image
  const flameGifSize = 280 // Increased size for the GIF
  const flameRadius = flameGifSize / 2 // Radius of the flame GIF
  const innerRayWidth = 30 // Width of the ray near the flame (narrow end)
  const outerRayWidth = 120 // Width of the ray at its outer edge (wide end)
  const rayLength = 180 // Length of the ray from inner to outer edge
  const outerRadius = flameRadius + rayLength // Max distance from center for ray tip
  ...
  const renderRays = () => {
    ...
    for (let i = 0; i < NUM_RAYS; i++) {
      ...
      // Points are calculated relative to the center of the flame (0,0)
      // Point 1 (inner-left)
      ...
      rays.push(
        <Polygon
          key={`ray-polygon-${i}`}
          points={points}
          fill={`url(#rayGradient${i})`} // Use unique gradient ID for each ray
    ...
    >
      ...
      >
        ...
        <View style={styles.flameContainer}>
          {/* SVG for rays with linear gradient and transform */}
          <Animated.View style={[styles.raysSvgWrapper, { transform: [{ rotate: rayRotationDegrees }] }]}>
            <Svg
              height={outerRadius * 2} // Max height needed for the rays
              width={outerRadius * 2} // Max width needed for the rays
              viewBox={`-${outerRadius} -${outerRadius} ${outerRadius * 2} ${outerRadius * 2}`} // Centered viewBox
                ...
                {Array.from({ length: NUM_RAYS }).map((_, i) => {
                  ...
                  return (
                    <SVGLinearGradient
                      ...
                      y1="0%"
                      x2="100%"
                      y2="0%" // Define a horizontal gradient
                      gradientUnits="objectBoundingBox" // Apply gradient relative to the object's bounding box
                      gradientTransform={`rotate(${angle})`} // Rotate the gradient to align with the ray
                    >
                      {/* Gradient from opaque yellow to transparent yellow */}
                      <Stop offset="0%" stopColor="rgba(255, 200, 0, 0.8)" /> {/* Opaque at inner edge */}
                      <Stop offset="100%" stopColor="rgba(255, 200, 0, 0)" /> {/* Transparent at outer edge */}
                    </SVGLinearGradient>
                  )
                })}
              </Defs>
...
const styles = StyleSheet.create({
  ...
  raysSvgWrapper: {
    ...
    justifyContent: "center",
    alignItems: "center",
    zIndex: 1, // Ensure rays are behind the flame GIF
  },
  raysSvg: {
    // No specific styles needed here, size and viewBox are set in component
  },
  flameGif: {
    width: 280, // Increased size for the GIF
    height: 280, // Increased size for the GIF
    position: "absolute",
    zIndex: 2, // Ensure flame GIF is on top of rays
  },
  actionButton: {
    borderRadius: 25,
    overflow: "hidden",
