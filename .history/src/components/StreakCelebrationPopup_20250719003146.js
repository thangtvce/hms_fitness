...
const NUM_RAYS = 8 // Reduced number of rays for wider, softer appearance
...
const StreakCelebrationPopup = ({ isVisible, streakCount, onClose }) => {
  ...
  const renderRays = () => {
    const rays = []
    const rayLength = 120 // Length of each ray segment
    const rayWidth = 40 // Width of each ray segment
    const startOffset = 100 // Distance from the center of the flame where rays start
        ...
        <Animated.View
          ...
          style={[
            styles.ray,
            {
              width: rayWidth,
              height: rayLength,
              backgroundColor: "rgba(255, 200, 0, 0.4)", // Warm yellow/orange with transparency
              position: "absolute",
              left: "50%",
              top: "50%",
              ...
              transform: [
                { rotate: rotation }, // Rotate each ray individually
                { translateY: startOffset }, // Move it outwards from the center
                { rotate: rayRotationDegrees }, // Apply continuous rotation to the whole system
              ],
              shadowColor: "#FFD700", // Add shadow for glow effect
...
const styles = StyleSheet.create({
  ...
  ray: {
    position: "absolute",
    backgroundColor: "rgba(255,200,0,0.4)", // Warm yellow/orange with transparency
    borderRadius: 20, // Rounded corners for softer look
  },
  flameGif: {
    width: 200,
    height: 200,
