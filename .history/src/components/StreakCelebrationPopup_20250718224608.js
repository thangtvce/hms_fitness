...
import Svg, { Polygon, Defs, RadialGradient, Stop } from "react-native-svg" // Import RadialGradient
...
const StreakCelebrationPopup = ({ isVisible, streakCount, onClose }) => {
  ...
  const flameGifSize = 250 // Matches flameGif width/height
  const flameRadius = flameGifSize / 2 // Radius of the flame GIF
  const innerRayWidth = 20 // Width of the ray near the flame (narrow end)
  const outerRayWidth = 100 // Width of the ray at its outer edge (wide end)
    ...
    for (let i = 0; i < NUM_RAYS; i++) {
      ...
      rays.push(
        <Polygon
          key={`ray-polygon-${i}`}
          points={points}
          fill="url(#rayRadialGradient)" // Use the shared radial gradient
    ...
    >
      ...
      >
        ...
        <View style={styles.flameContainer}>
          {/* SVG for rays with radial gradient */}
          <Animated.View style={[styles.raysSvgWrapper, { transform: [{ rotate: rayRotationDegrees }] }]}>
            <Svg
              height={flameRadius * 2 + rayLength * 2} // Max height needed for the rays
              width={flameRadius * 2 + rayLength * 2} // Max width needed for the rays
              viewBox={`-${flameRadius + rayLength} -${flameRadius + rayLength} ${flameRadius * 2 + rayLength * 2} ${flameRadius * 2 + rayLength * 2}`} // Centered viewBox
              style={styles.raysSvg}
            >
              <Defs>
                <RadialGradient id="rayRadialGradient" cx="0" cy="0" r="100%" fx="0" fy="0">
                  {/* Gradient from opaque yellow at center to transparent at outer edge */}
                  <Stop offset="0%" stopColor="rgba(255, 200, 0, 0.8)" /> {/* Opaque at center */}
