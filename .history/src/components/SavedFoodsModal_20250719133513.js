      ...
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.3)' }}>
        <View style={{ flex: 1, backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, marginTop: 48, overflow: 'hidden' }}>
          ...
          <View style={{ flex: 1, paddingHorizontal: 20, paddingBottom: 20 }}>
                ...
                <View key={food.foodId ? `saved-${food.foodId}` : `${food.name}-${idx}`}
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={{ fontSize: 14, color: '#64748B' }}>
                      {food.kcal} kcal
                      {(food.servingSize > 1 || food.portionSize > 1) && (
                        <>
                          {food.servingSize > 1 && ` • S${food.servingSize}`}
                          {food.portionSize > 1 && ` • P${food.portionSize}`}
                        </>
                      )}
                    </Text>
                  </View>
            ...
            {editIdx !== null && (
              <View style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: '35%', backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, borderWidth: 1, borderColor: '#E5E7EB', shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 8, elevation: 6, padding: 20, alignItems: 'center', justifyContent: 'flex-start' }}>
                <View style={{ width: '100%', maxWidth: 340 }}>
                  <Text style={{ fontSize: 18, fontWeight: 'bold', marginBottom: 6, textAlign: 'center', color: '#1E293B' }}>Edit Portion</Text>
                  <Text style={{ fontSize: 16, fontWeight: '500', marginBottom: 16, textAlign: 'center', color: '#0056d2' }}>{savedFoods[editIdx]?.name}</Text>
                  {/* Serving size row */}
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
                    <Text style={{ fontSize: 14, color: '#64748B', width: 100 }}>Serving size</Text>
                    <TouchableOpacity
                      style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: '#F1F5F9', alignItems: 'center', justifyContent: 'center', marginRight: 6 }}
                      onPress={() => setEditServing(prev => String(Math.max(1, (parseInt(prev) || 1) - 1)))}
                    >
                      <Feather name="minus" size={18} color="#64748B" />
                    </TouchableOpacity>
                    <Text style={{ width: 40, textAlign: 'center', fontSize: 18, color: '#1E293B', fontWeight: 'bold' }}>{editServing}</Text>
                    <TouchableOpacity
                      style={{ width: 32, height: 32, borderRadius: 16, backgroundColor: '#F1F5F9', alignItems: 'center', justifyContent: 'center', marginLeft: 6 }}
                      onPress={() => setEditServing(prev => String((parseInt(prev) || 1) + 1))}
                    >
