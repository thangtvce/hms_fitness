...
const StreakCelebrationPopup = ({ isVisible, streakCount, onClose }) => {
  ...
  const flameRadius = flameGifSize / 2.5 // Reduced to bring rays closer to flame
  const innerRayWidth = 25 // Width of the ray near the flame (narrow end)
  const outerRayWidth = 80 // Width of the ray at its outer edge (wide end) - reduced
  const rayLength = 120 // Reduced length to bring rays closer
  const outerRadius = flameRadius + rayLength
              ...
              <Defs>
                {Array.from({ length: NUM_RAYS }).map((_, i) => {
                  ...
                  return (
                    ...
                    >
                      {/* Inner part (near flame) - darker and more opaque */}
                      <Stop offset="0%" stopColor="rgba(255, 193, 7, 0.9)" />
                      <Stop offset="30%" stopColor="rgba(255, 193, 7, 0.7)" />
                      <Stop offset="60%" stopColor="rgba(255, 193, 7, 0.4)" />
                      {/* Outer part - very transparent */}
