...
export default function WeightAnalystScreen({ navigation }) {
  ...
  const filteredHistory = (() => {
    // Use the same filter as groupData, but return the raw items
    if (filter === "daily") {
      return history
    } else if (filter === "weekly") {
      // Group by week, use last entry of each week
      const weekMap = {}
      ...
      return Object.values(weekMap).map(arr => arr[arr.length - 1])
    } else if (filter === "monthly") {
      // Group by month, use last entry of each month
      const monthMap = {}
      history.forEach(item => {
        const d = new Date(item.recordedAt)
        const key = `${d.getFullYear()}-${d.getMonth() + 1}`
      ...
      return Object.values(monthMap).map(arr => arr[arr.length - 1])
    }
    return history
  })()

  const statsForFilter = calculateStats(filteredHistory)
      ...
      <View style={[styles.container, { paddingTop: 80 }]}> 
        <View style={styles.filterContainer}>
          {FILTER_OPTIONS.map((option) => (
            <TouchableOpacity
              key={option.key}
              style={[styles.filterButton, filter === option.key && styles.filterButtonActive, option.key === 'daily' && styles.filterButtonFirst, option.key === 'monthly' && styles.filterButtonLast]}
        ...
        {/* Stats section below filter */}
        <View style={styles.statsContainer}>
          <View style={styles.statCard}>
            <Text style={styles.statLabel}>Current</Text>
            <Text style={styles.statValue}>{statsForFilter.current} kg</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statLabel}>Average</Text>
            <Text style={styles.statValue}>{statsForFilter.average} kg</Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statLabel}>Change</Text>
            <Text
              style={[styles.statValue, statsForFilter.change > 0 ? styles.statValueUp : statsForFilter.change < 0 ? styles.statValueDown : null]}
            >
              {statsForFilter.change > 0 ? "+" : ""}{statsForFilter.change} kg
            </Text>
          </View>
        </View>
