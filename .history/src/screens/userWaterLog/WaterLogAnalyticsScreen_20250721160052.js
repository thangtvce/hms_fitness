  ...
  const generateChartData = (records, filterObj, startDate, endDate) => {
    ...
    if (filterObj.id === 'daily') {
      ...
      labels = sortedWeeks.map(k => {
        ...
        // Correct usage: set both isoWeekYear and isoWeek in one chain
        // Defensive: check if isoWeek and isoWeekYear are available
        if (typeof dayjs().isoWeek !== 'function' || typeof dayjs().isoWeekYear !== 'function') {
          return '';
        }
