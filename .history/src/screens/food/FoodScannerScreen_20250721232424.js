...
export default function FoodScannerScreen({ navigation }) {
    ...
    const takePicture = async () => {
        if (cameraRef.current) {
            try {
                ...
                try {
                    const response = await apiFoodScannerService.scannerFood({ imageFile });
                    if (response.is_food_image) {
                        setAnalysisResult({
                            name: response.food_name,
                            emoji: getFoodEmoji(response.food_name),
                            calories: Math.round(response.predictions.calories),
                            weight: `${Math.round(response.predictions.total_weight)}g`,
                            ingredients: [
                                {
                                    name: `Fat`,
                                    calories: Math.round(response.predictions.fat),
                                    weight: '-',
                                    emoji: '🥛'
                                },
                                {
                                    name: `Carbs`,
                                    calories: Math.round(response.predictions.carbs),
                                    weight: '-',
                                    emoji: '🍞'
                                },
                                {
                                    name: `Protein`,
                                    calories: Math.round(response.predictions.protein),
                                    weight: '-',
                                    emoji: '🍗'
                                }
                            ]
                        ...
                        showErrorFetchAPI('No food detected in the image');
                ...
                try {
                    ...
                    if (response.is_food_image) {
                        setAnalysisResult({
                            name: response.food_name,
                            emoji: getFoodEmoji(response.food_name),
                            calories: +response.predictions.calories.toFixed(3),
                            weight: `${+response.predictions.total_weight.toFixed(3)}g`,
                            ingredients: [
                                {
                                    name: `Fat`,
                                    calories: +response.predictions.fat.toFixed(3),
                                    weight: '-',
                                    emoji: '🥛'
                                },
                                {
                                    name: `Carbs`,
                                    calories: +response.predictions.carbs.toFixed(3),
                                    weight: '-',
                                    emoji: '🍞'
                                },
                                {
                                    name: `Protein`,
                                    calories: +response.predictions.protein.toFixed(3),
                                    weight: '-',
                                    emoji: '🍗'
                                }
                            ]
    ...
    const getFoodEmoji = (foodName) => {
        ...
        return emojiMap[foodName.toLowerCase()] || emojiMap.default;
    };
