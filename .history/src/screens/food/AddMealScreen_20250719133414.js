          ...
          return (
            <View key={key} style={styles.foodRow}>
              ...
              <Text style={styles.foodKcal}>
                {item.kcal} kcal
                {(() => {
                  const saved = savedFoods.find(f => (f.foodId && item.foodId && f.foodId === item.foodId) || (!f.foodId && f.name === item.name));
                  if (saved && (saved.portionSize > 1 || saved.servingSize > 1)) {
                    let txt = '';
                    if (saved.servingSize > 1) txt += ` • S${saved.servingSize}`;
                    if (saved.portionSize > 1) txt += ` • P${saved.portionSize}`;
                    return txt;
                  }
                  return '';
                })()}
                  ...
                  onPress={async () => {
                    ...
                    try {
                      ...
                      if (idx === -1) {
                        ...
                        // Tăng portionSize lên 1 (hoặc servingSize nếu muốn)
                        let updated = [...foods];
                        let old = updated[idx];
      ...
      <FoodDetailsOverlay
        ...
        onAdd={async () => {
          ...
          try {
            ...
            if (!exists) {
              let fullFood = { ...foodOverlayData };
              const found = frequentFoods.find(f => f.name === foodOverlayData.name);
              if (found) {
                fullFood = {
                  ...fullFood,
                  foodId: found.foodId,
