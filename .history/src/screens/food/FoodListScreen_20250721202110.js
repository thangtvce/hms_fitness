...
const FoodListScreen = () => {
  ...
  // Custom shimmer card for food list loading
  const renderFoodShimmer = (layoutMode) => {
    const shimmerCount = layoutMode === 1 ? 4 : layoutMode === 2 ? 6 : layoutMode === 3 ? 9 : 12;
    const itemWidth =
      layoutMode === 1 ? "100%" : layoutMode === 2 ? "48%" : layoutMode === 3 ? "31%" : layoutMode === 4 ? "23%" : "23%";
    const imageHeight =
      layoutMode === 1 ? 180 : layoutMode === 2 ? 140 : layoutMode === 3 ? 120 : layoutMode === 4 ? 100 : 100;
    return (
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'flex-start', paddingHorizontal: 8, marginTop: 8 }}>
        {Array.from({ length: shimmerCount }).map((_, idx) => (
          <View key={idx} style={{ width: itemWidth, marginRight: layoutMode > 1 ? '2%' : 0, marginBottom: 16 }}>
            <View style={{ backgroundColor: '#fff', borderRadius: 16, overflow: 'hidden', elevation: 2 }}>
              <ShimmerPlaceholder style={{ height: imageHeight, width: '100%' }} borderRadius={16} />
              <View style={{ padding: layoutMode > 2 ? 12 : 20 }}>
                <ShimmerPlaceholder style={{ height: 14, width: '60%', marginBottom: 8 }} borderRadius={8} />
                <ShimmerPlaceholder style={{ height: 10, width: '40%', marginBottom: 8 }} borderRadius={8} />
                <ShimmerPlaceholder style={{ height: 10, width: '30%' }} borderRadius={8} />
              </View>
            </View>
          </View>
        ))}
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.contentContainer}>
        {error && !refreshing ? (
          ...
          renderFoodShimmer(layoutMode === "quick" ? 1 : layoutMode)
        ) : (
          <>
            <FlatList
              data={foods}
