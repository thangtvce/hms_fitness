...
const FavoriteFoodsScreen = ({ navigation }) => {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background || "#F8FAFC" }]}> 
      ...
      {/* Overlay Modal for FoodDetailsScreen */}
      <Modal
        visible={modalVisible && selectedFood && (selectedFood.foodDetails?.foodId || selectedFood.foodId)}
        animationType="slide"
        transparent
        onRequestClose={handleCloseModal}
      >
        <Pressable style={styles.overlayBackground} onPress={handleCloseModal} />
        <View style={styles.overlayContainer}>
          {selectedFood && (selectedFood.foodDetails?.foodId || selectedFood.foodId) && (
            <FoodDetailsScreen
              route={{ params: { food: selectedFood } }}
