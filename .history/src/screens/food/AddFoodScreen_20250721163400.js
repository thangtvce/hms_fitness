...
const AddFoodScreen = ({ visible, food: propFood, mealType = "Breakfast", onClose }) => {
  const { user } = useAuth()
  const navigation = useNavigation()
  const route = useRoute()
  const food = propFood || route.params?.food || {}
  ...
  return (
    <Modal visible={visible !== undefined ? visible : true} transparent animationType="slide" onRequestClose={handleClose}>
      <View style={styles.overlay}>
        <View style={styles.container}>
          ...
          {/* Removed Serving Size control section as requested */}
...
const styles = StyleSheet.create({
  ...
  mealTypeButton: {
    flex: 1,
    paddingVertical: 12,
    marginHorizontal: 0.5, // reduced margin for closer buttons
