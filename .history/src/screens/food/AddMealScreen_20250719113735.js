...
import FoodDetailsOverlay from 'components/FoodDetailsOverlay';
...
function AddMealScreen({ navigation, route }) {
  ...
  // For badge animation (optional, can be removed if not needed)
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.3)' }}>
          <View style={{ flex: 1, backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, marginTop: 48, overflow: 'hidden' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 20, paddingBottom: 8 }}>
              ...
              <TouchableOpacity onPress={() => setShowSavedFoodsModal(false)} style={{ padding: 8, marginRight: -8, marginTop: -8 }}>
          ...
          return (
            <View key={key} style={styles.foodRow}>
              <TouchableOpacity style={{ flex: 1 }} onPress={() => {
                setEditFood({ ...item, idx });
                setEditServing(item.servingSize ? String(item.servingSize) : '1');
                setEditPortion(item.portionSize ? String(item.portionSize) : '1');
              }}>
      ...
      {/* Edit Food Modal (for both saved and list foods) */}
      <Modal
        visible={!!editFood}
        transparent
        animationType="fade"
              ...
              <>
                ...
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    setEditLoading(true);
                    try {
                      // If editing a saved food
                      if (editFood.idx !== undefined && savedFoods[editFood.idx]) {
                        let updatedFoods = [...savedFoods];
                        const idx = editFood.idx;
                        const oldFood = updatedFoods[idx];
                        const serving = Number(editServing) || 1;
                        ...
                        // If editing a food from the list, just update the overlayData and open overlay
                        let updated = { ...editFood };
                        const serving = Number(editServing) || 1;
                        const portion = Number(editPortion) || 1;
                        let baseKcal = updated._baseKcal || updated.baseKcal || updated.kcal || 0;
                        if (!updated._baseKcal && updated.kcal) {
                          baseKcal = updated.kcal / ((updated.servingSize || 1) * (updated.portionSize || 1));
                        }
                        const newKcal = Math.round(baseKcal * serving * portion);
                        updated = {
                          ...updated,
