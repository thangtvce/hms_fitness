...
const MySubscriptionScreen = ({ navigation }) => {
    ...
    const handleSubscriptionPress = async (item) => {
        try {
            const rating = await apiSubscriptionService.hasUserRatedSubscription(user.userId, item.subscriptionId);
            if (rating) {
                // Hiển thị thông tin đánh giá
                Alert.alert(
                    "Đã đánh giá",
                    `Điểm: ${rating.rating} ⭐\nNhận xét: ${rating.feedbackText || 'Không có nhận xét'}\nNgày: ${rating.createdAt?.slice(0,10)}`,
                    [
                        { text: "Xem chi tiết", onPress: () => navigation.navigate("SubscriptionReviewDetailScreen", { subscription: item }) },
                        { text: "Đóng", style: "cancel" }
                    ]
                );
            } else {
                navigation.navigate("SubscriptionReviewDetailScreen", { subscription: item });
            }
        } catch (err) {
