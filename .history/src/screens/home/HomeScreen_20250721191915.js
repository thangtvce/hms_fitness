...
export default function HomeScreen({ navigation }) {
  ...
  const [weightTimeFrame, setWeightTimeFrame] = useState("3m") // Keep for internal filtering, but UI removed
    ...
    () => [
      {
        ...
        icon: "moon",
        route: null, // Show Coming Soon!
      },
      {
        title: "Recipes",
        subtitle: "Cook, eat, log, repeat",
        icon: "book-open", // Changed from 'list' to 'book-open' for recipes
        route: "Food", // Assuming a route for recipes
      },
      {
        title: "Workouts",
        subtitle: "Sweating is self-care",
        icon: "activity", // Use valid Feather icon for workouts
        route: "WorkoutListScreen",
      },
      {
        title: "Sync up",
        subtitle: "Link apps & devices",
        icon: "refresh-cw", // Changed from 'activity' to 'refresh-cw' for sync
        route: "SyncUpScreen", // Assuming a route for sync
      },
      {
        title: "Friends",
      ...
      },
      {
        ...
        icon: "message-square", // Changed from 'users' to 'message-square' for community
        route: "ActiveGroups",
      },
    ],
    [],
    ...
    return (
      ...
      >
        <View style={styles.discoverIconContainerNew}>
          <Feather name={item.icon} size={26} color="#4F46E5" />
        </View>
        <Text
          style={[
    ...
    <View style={[styles.container, { backgroundColor: colors.background || "#F8FAFC" }]}> 
      ...
      >
        ...
        <Animated.View style={[styles.section, { opacity: 1, transform: [{ translateY: 0 }] }]}> 
          ...
          <View style={[styles.calendarCard, { backgroundColor: 'transparent', borderColor: 'transparent', elevation: 0, shadowOpacity: 0 }]}> 
            {/* Shimmer placeholder for calendar - no extra white background */}
            <ShimmerCard height={100} style={{ borderRadius: 12, width: '100%' }} />
          </View>
        </Animated.View>
        <Animated.View style={[styles.section, { opacity: 1, transform: [{ translateY: 0 }] }]}> 
          {/* Nutrition Summary Card shimmer */}
          <ShimmerCard height={140} style={{ marginVertical: 12, borderRadius: 16, width: '100%' }} />
        </Animated.View>
        <Animated.View style={[styles.section, { opacity: 1, transform: [{ translateY: 0 }] }]}> 
          {/* Nutrition Section shimmer */}
          <ShimmerCard height={220} style={{ marginVertical: 12, borderRadius: 16, width: '100%' }} />
        </Animated.View>
        <Animated.View style={[styles.sectionWeight, { opacity: 1, transform: [{ translateY: 0 }] }]}> 
          {/* Weight Stats shimmer */}
          <ShimmerCard height={180} style={{ marginVertical: 12, borderRadius: 16, width: '100%' }} />
        </Animated.View>
        <Animated.View style={[styles.section, { opacity: 1, transform: [{ translateY: 0 }], marginTop: 30, marginBottom: 64, paddingHorizontal: 16, paddingVertical: 16, width: '100%' }]}> 
          ...
          {/* Discover shimmer grid */}
          <ShimmerCard height={100} style={{ marginVertical: 12, borderRadius: 16, width: '100%' }} />
        </Animated.View>
      </ScrollView>
    </View>
  )
...
const styles = StyleSheet.create({
  ...
  discoverGrid: {
    ...
    gap: SPACING / 2, // Gap between items
  },
  discoverGridItem: {
    width: (width - SPACING * 2 - SPACING / 2) / 2, // Two items per row with spacing
    padding: SPACING / 2,
    borderRadius: 12,
    marginBottom: SPACING / 2, // Vertical spacing between rows
    alignItems: "center",
