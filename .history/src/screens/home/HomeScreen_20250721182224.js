...
export default function HomeScreen({ navigation }) {
  ...
  const [isFoodCarouselPaused, setIsFoodCarouselPaused] = useState(false)
  ...
  useEffect(() => {
    ...
    if (isFoodCarouselPaused) return
  ...
  }, [foodImagesRow1.length, isFoodCarouselPaused])
  ...
  }, [foodImagesRow2.length, isFoodCarouselPaused])
  ...
  const renderFoodImageItem = (food, idx) => {
    ...
    return (
      <TouchableOpacity
        key={foodKey}
        style={styles.foodCarouselItem}
        activeOpacity={0.8}
        onPressIn={() => setIsFoodCarouselPaused(true)}
        onPressOut={() => setIsFoodCarouselPaused(false)}
      >
        <Image
...
Discover nutritious foods                </Text>
    ...
    Browse foods picked by GetFit to support your health and goals.
                ...
                >
                  <Text style={[styles.viewMealPlansButtonText, { color:  "#0056d2" }]}>Explore foods <Feather name="arrow-right" size={14} color={ "#0056d2"} /></Text>
                </TouchableOpacity>
              </View>
            </Animated.View>
          </>
