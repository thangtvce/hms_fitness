...
export default function HomeScreen({ navigation }) {
  ...
  const foodImagesRow2 = useMemo(() => {
    if (foodImages.length === 0) return []
    // Offset the array by half for interleaved effect
    const n = foodImages.length
    const offset = Math.floor(n / 2)
    const shifted = [...foodImages.slice(offset), ...foodImages.slice(0, offset)]
  ...
  const NutritionSection = () => {
    ...
    // Giả sử dashboardData.nutritionLogsToday đã được set ở fetchUserData, nếu chưa thì thì lấy từ dashboardData.nutritionLogs và filter theo ngày
  ...
  // New Trainer Service Ad Banner Component
  const TrainerServiceAdBanner = () => {
    return (
      <TouchableOpacity
        style={[styles.trainerServiceAdBanner, MODERN_SHADOW]}
        onPress={() => handleNavigation("TrainerServiceScreen")} // Assuming a screen for trainer services
        activeOpacity={0.9}
      >
        <LinearGradient
          colors={["#4F46E5", "#6366F1"]} // Blue/purple gradient
          style={styles.trainerServiceAdBannerGradient}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 0 }}
        >
          <View style={styles.trainerServiceAdBannerContent}>
            <View style={styles.trainerServiceAdBannerTextContainer}>
              <Text style={styles.trainerServiceAdBannerSmallTitle}>Personalized Training</Text>
              <Text style={styles.trainerServiceAdBannerTitle}>Achieve Your Goals</Text>
              <TouchableOpacity
                style={styles.trainerServiceAdBannerButton}
                onPress={() => handleNavigation("FindTrainerScreen")} // Navigate to a screen to find trainers
              >
                <Text style={styles.trainerServiceAdBannerButtonText}>START YOUR JOURNEY</Text>
              </TouchableOpacity>
              <Text style={styles.trainerServiceAdBannerSubtitle}>
                Get expert guidance and custom workout plans from certified trainers.
              </Text>
            </View>
            <View style={styles.trainerServiceAdBannerIcons}>
              <Feather name="dumbbell" size={40} color="rgba(255,255,255,0.2)" style={styles.trainerServiceAdIcon1} />
              <Feather name="award" size={30} color="rgba(255,255,255,0.2)" style={styles.trainerServiceAdIcon2} />
              <Feather name="star" size={25} color="rgba(255,255,255,0.2)" style={styles.trainerServiceAdIcon3} />
            </View>
          </View>
        </LinearGradient>
      </TouchableOpacity>
      ...
      >
        {dashboardData && (
          <>
            ...
            {/* New Trainer Service Ad Banner */}
            <Animated.View style={[styles.section, { opacity: fadeAnim, transform: [{ translateY: translateY }] }]}>
              <TrainerServiceAdBanner />
            </Animated.View>
            ...
            >
              <View style={[styles.sectionHeader, { paddingHorizontal: SPACING }]}>
                <Text style={[styles.sectionTitle, { color: colors.sectionTitle || "#1E293B" }]}>Foods</Text>
              </View>
              {foodLoading ? (
...
const styles = StyleSheet.create({
  ...
  // Trainer Service Ad Banner Styles
  trainerServiceAdBanner: {
    borderRadius: 12,
    overflow: "hidden",
    marginTop: SPACING, // Add some space above it
  },
  trainerServiceAdBannerGradient: {
    padding: SPACING,
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  },
  trainerServiceAdBannerContent: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
    flex: 1,
  },
  trainerServiceAdBannerTextContainer: {
    flex: 1,
    marginRight: SPACING / 2,
  },
  trainerServiceAdBannerSmallTitle: {
    fontSize: 14,
    color: "rgba(255,255,255,0.8)",
    marginBottom: 4,
  },
  trainerServiceAdBannerTitle: {
    fontSize: 22,
    fontWeight: "bold",
    color: "#FFFFFF",
    marginBottom: SPACING / 2,
  },
  trainerServiceAdBannerButton: {
  ...
  trainerServiceAdBannerButtonText: {
    fontSize: 14,
    fontWeight: "bold",
    color: "#4F46E5", // Primary blue color
  },
  trainerServiceAdBannerSubtitle: {
    fontSize: 12,
    color: "rgba(255,255,255,0.7)",
    lineHeight: 18,
  },
  trainerServiceAdBannerIcons: {
    position: "relative",
    width: 80, // Container for icons
    height: 80,
  },
  trainerServiceAdIcon1: {
    position: "absolute",
    top: 0,
    right: 0,
    transform: [{ rotate: "15deg" }],
  },
  trainerServiceAdIcon2: {
    position: "absolute",
    bottom: 10,
    left: 0,
    transform: [{ rotate: "-20deg" }],
  },
  trainerServiceAdIcon3: {
    position: "absolute",
    top: 30,
    left: 20,
