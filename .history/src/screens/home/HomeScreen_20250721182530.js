...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const placeholderFoods = [
      { foodId: "p1", image: "/placeholder.svg?height=100&width=100" },
      { foodId: "p2", image: "/placeholder.svg?height=100&width=100" },
      { foodId: "p3", image: "/placeholder.svg?height=100&width=100" },
      { foodId: "p4", image: "/placeholder.svg?height=100&width=100" },
      { foodId: "p5", image: "/placeholder.svg?height=100&width=100" },
      { foodId: "p6", image: "/placeholder.svg?height=100&width=100" },
      { foodId: "p7", image: "/placeholder.svg?height=100&width=100" },
    ]
    setFoodImages(placeholderFoods) // Initialize with placeholders
    ...
    const fetchFoods = async () => {
      ...
      try {
        ...
        if (res?.data?.foods && res.data.foods.length > 0) {
          ...
          setFoodImages(placeholderFoods) // Fallback to placeholders if no data
        }
      } catch (e) {
        setFoodError("Could not load foods")
        setFoodImages(placeholderFoods) // Fallback to placeholders on error
      } finally {
    ...
    if (foodImages.length > 0) {
      const interval = setInterval(() => {
        setAutoScrollIndex((prevIndex) => {
          const nextIndex = (prevIndex + 1) % foodImages.length
  ...
  const renderFoodImageItem = (food) => (
    <TouchableOpacity key={food.foodId} style={styles.foodCarouselItem} activeOpacity={0.8}>
      ...
      {/* No text below image as per design */}
    </TouchableOpacity>
  )
          ...
          <>
            ...
            >
              ...
              {foodLoading ? (
                ...
                >
                  {foodImages.map(renderFoodImageItem)}
                </ScrollView>
              )}
