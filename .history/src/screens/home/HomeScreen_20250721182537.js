...
export default function HomeScreen({ navigation }) {
  ...
  // Render food image grid item
  const renderFoodImageItem = (food) => (
    <View key={food.foodId} style={styles.foodImageGridItem}>
      <Image
        source={{ uri: food.image }}
        style={styles.foodImage}
        resizeMode="cover"
      />
      <Text style={styles.foodImageName} numberOfLines={1} ellipsizeMode="tail">{food.foodName}</Text>
    </View>
  )
