...
export default function HomeScreen({ navigation }) {
  ...
  const foodImagesRow2 = useMemo(() => {
    if (foodImages.length === 0) return []
    // Duplicate and reverse or offset for a different scroll effect
    const reversedFoods = [...foodImages].reverse()
    return [...reversedFoods, ...reversedFoods, ...reversedFoods]
  }, [foodImages])
  ...
  useEffect(() => {
    ...
    }, 20) // Adjust speed here (smaller value = faster)

    return () => clearInterval(scrollInterval1)
  }, [foodImagesRow1.length])

  // Auto-scrolling logic for row 2 (scrolling in opposite direction or different speed)
  useEffect(() => {
    ...
    // Start row 2 slightly offset or scroll in reverse
    scrollX2.setValue(totalWidth) // Start at the end of the first set for reverse scroll

    const scrollInterval2 = setInterval(() => {
      scrollX2.setValue(scrollX2._value - 0.8) // Decrement by a small value for smooth animation, slightly slower
      if (scrollX2._value <= 0) {
        // If scrolled past the beginning of one full set, reset without animation
  ...
  // Render food image carousel item
  const renderFoodImageItem = (food) => (
    <TouchableOpacity key={food.foodId} style={styles.foodCarouselItem} activeOpacity={0.8}>
          ...
          <>
            ...
            >
              ...
              {foodLoading ? (
                ...
                <View>
                  <ScrollView
                    ...
                    contentContainerStyle={styles.foodCarouselContainer}
                  >
                    {foodImagesRow1.map(renderFoodImageItem)}
                  </ScrollView>
                  <ScrollView
                    horizontal
                    showsHorizontalScrollIndicator={false}
                    ref={scrollViewRef2}
                    scrollEventThrottle={16}
                    contentContainerStyle={[styles.foodCarouselContainer, { marginTop: SPACING / 2 }]} // Add vertical spacing
                  >
                    {foodImagesRow2.map(renderFoodImageItem)}
                  </ScrollView>
                </View>
              )}
              ...
              <View style={styles.dietPlanSection}>
                ...
                <Text style={[styles.dietPlanSubtitle, { color: colors.textSecondary || "#64748B" }]}>
                  Here's your personalised diet plan specially designed by GetFit nutritionists!
                </Text>
                <TouchableOpacity
                  style={styles.viewMealPlansButton}
                  onPress={() => handleNavigation("UserGoalPlansScreen")}
                >
                  <Text style={[styles.viewMealPlansButtonText, { color: colors.success || "#10B981" }]}>
                    View meal plans <Feather name="arrow-right" size={14} color={colors.success || "#10B981"} />
                  </Text>
                </TouchableOpacity>
              </View>
            </Animated.View>
...
const styles = StyleSheet.create({
  ...
  foodCarouselItem: {
    width: 100, // Overall size of the circular plate
    height: 100,
    borderRadius: 50, // Make it circular
    justifyContent: "center",
    alignItems: "center",
    marginRight: SPACING / 2, // Spacing between items
    backgroundColor: "#FFFFFF", // This is the plate color
    ...MODERN_SHADOW, // Apply modern shadow
    overflow: "hidden", // Clip content to border radius
  },
  foodImage: {
    width: 90, // Slightly smaller than the container to show the white plate border
    height: 90,
    borderRadius: 45, // Half of width/height to make it circular
    backgroundColor: "#F3F4F6", // Placeholder background if image fails to load
  },
