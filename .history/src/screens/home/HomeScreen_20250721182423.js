...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const fetchFoods = async () => {
      ...
      setFoodError("")
      try {
        const res = await foodService.getAllActiveFoods({ pageNumber: 1, pageSize: 100 })
        let foods = []
        if (res?.data?.foods && Array.isArray(res.data.foods)) {
          // Ưu tiên food có hình, sau đó mới đến food không có hình
          const foodsWithImage = res.data.foods.filter(f => f.image && typeof f.image === 'string' && f.image.trim() !== '')
          const foodsWithoutImage = res.data.foods.filter(f => !f.image || f.image.trim() === '')
          foods = [...foodsWithImage, ...foodsWithoutImage]
        }
        // Nếu có food có hình thì chỉ lấy những food này, nếu không có thì lấy staticFoodImages
        if (foods.length > 0 && foods.some(f => f.image && f.image.trim() !== '')) {
          setFoodImages(
            foods.map(f => ({
              foodId: f.foodId || f.id || f._id || f.name || Math.random().toString(),
              image: f.image && f.image.trim() !== '' ? f.image : require("../../../assets/images/workout-hero.jpeg"),
            }))
          )
        } else {
          setFoodImages(staticFoodImages)
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    }, 100) // Slower scroll (higher value = slower)
  ...
  const renderFoodImageItem = (food, idx) => {
    ...
    return (
      <TouchableOpacity key={foodKey} style={styles.foodCarouselItem} activeOpacity={0.8}>
        <Image
          source={imgError ? require("../../../assets/images/workout-hero.jpeg") : { uri: food.image }}
