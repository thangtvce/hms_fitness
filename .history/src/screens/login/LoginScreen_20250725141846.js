...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Log trạng thái profile
      console.log('[DEBUG] isProfileCompleted:', isProfileCompleted);
      ...
      if (isProfileCompleted === false) {
        // Thông báo cho user biết cần tạo profile
        console.log('[LOGIN NAVIGATE] Chuyển tới ProfileSteps');
        showErrorFetchAPI("Bạn cần hoàn thành hồ sơ cá nhân trước khi sử dụng ứng dụng.");
        // Đọc trạng thái lưu trong AsyncStorage nếu có
        let savedStep = 0;
        let savedFormData = {};
        try {
        ...
        const targetScreen = hasRole("Trainer") ? "AdminDashboard" : "HomeScreen"
        console.log(`[LOGIN NAVIGATE] Chuyển tới ${targetScreen}`);
  ...
  const handleGoogleLogin = async (token) => {
    ...
    try {
      ...
      const targetScreen = hasRole("Admin") ? "AdminDashboard" : "HomeScreen"
      navigation.replace(targetScreen)
    } catch (error) {
