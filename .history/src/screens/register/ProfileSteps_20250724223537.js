...
export default function ProfileSteps({
  ...
  // Lưu formData và step vào AsyncStorage mỗi khi thay đổi
  ...
  const handleCompleteProfile = async () => {
    ...
    if (missingFields.length > 0) {
      ...
      showErrorFetchAPI(`Vui lòng nhập đầy đủ thông tin sau: ${missingLabels}`);
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // Lấy các trường bắt buộc từ userData
      ...
      if (!fullName) missingUserFields.push("Họ tên");
      if (!email) missingUserFields.push("Email");
      if (!phone) missingUserFields.push("Số điện thoại");

      // Validate định dạng email, phone nếu muốn (ở đây chỉ check rỗng)
      if (missingUserFields.length > 0) {
        setIsLoading(false);
        showErrorFetchAPI(`Tài khoản của bạn thiếu thông tin bắt buộc: ${missingUserFields.join(", ")}. Vui lòng cập nhật hồ sơ cá nhân trước khi hoàn thành.`);
        return;
      }
      ...
      const birthDate = formData?.age
        ...
        : undefined;
      // Đảm bảo gender đúng format 'Male', 'Female', 'Other' (chữ cái đầu viết hoa)
      ...
      // Always ensure userId, fullName, email, phone are set (formData first, fallback to userData/resolved userId)
      const userDto = {
...
if (error.response && error.response.data && error.response.data.message) {
  ...
  // Khi mount, khôi phục lại step và formData nếu có trong AsyncStorage
  useEffect(() => {
    (async () => {
      try {
...
const BodyFatStep = ({ formData, handleChange, error, onComplete }) => {
  const [bodyFat, setBodyFat] = useState(formData?.bodyFatPercentage ? String(formData.bodyFatPercentage) : "15");
    ...
    <View style={styles.stepContainer}>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          ...
          handleChange("bodyFatPercentage", value.toString());
          onComplete();
        }}
      >
