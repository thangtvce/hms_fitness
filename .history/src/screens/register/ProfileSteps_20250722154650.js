...
import { apiProfileService } from "services/profileService"
import CustomRuler from "components/CustomRuler"

const screenDimensions = Dimensions.get("window")
const { width, height } = screenDimensions || { width: 375, height: 667 }
...
export default function ProfileSteps({
  ...
  const handleCompleteProfile = async () => {
    ...
    try {
      ...
      // Call APIs
      await Promise.all([
        apiUserService.updateUser(user.id, userUpdateData),
        apiProfileService.registerProfile(profileData),
      ])
