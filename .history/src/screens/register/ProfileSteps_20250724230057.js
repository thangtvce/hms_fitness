  ...
  const validateCurrentStep = () => {
    ...
    switch (step) {
      ...
      case 4: // Activity Level
        const validActivityLevels = ["Sedentary", "Lightly Active", "Moderately Active", "Very Active", "Extremely Active"];
      ...
      case 5: // Fitness Goals
        const validFitnessGoals = ["Weight Loss", "Maintain", "Muscle Gain", "Improve Endurance", "Increase Strength", "Improve Flexibility"];
      ...
      case 7: // Dietary Preference
        const validDietaryPreferences = ["Standard", "Vegetarian", "Vegan", "Pescatarian", "Paleo", "Keto", "Mediterranean", "Low-carb", "Gluten-free", "Dairy-free"];
      ...
      case 8: // Body Fat
        ...
        if (!formData?.bodyFatPercentage || isNaN(bodyFat) || bodyFat < 3 || bodyFat > 60) {
...
const BodyFatStep = ({ formData, handleChange, error, onComplete }) => {
  ...
  const handleBodyFatChange = (text) => {
    ...
    if (!isNaN(value) && value >= 3 && value <= 60) {
      if (error) handleChange("bodyFatPercentage", cleaned); // trigger error clear in parent
    }
  };
  ...
  const handleComplete = () => {
    ...
    if (isNaN(value) || value < 3 || value > 60) {
      ...
      handleChange("bodyFatPercentage", ""); // set error in parent
      return;
    }
    onComplete();
  };
