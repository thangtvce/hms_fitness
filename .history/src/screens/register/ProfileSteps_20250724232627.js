...
export default function ProfileSteps({
  // Redirect to HomeScreen if user already has a profile
  useEffect(() => {
    const checkUserProfile = async () => {
      try {
        ...
        if (userDataObj && (userDataObj.hasProfile === true || userDataObj.hasProfile === 'true')) {
  ...
  // Defer conditional returns until after all hooks
...
const FitnessGoalStep = ({ formData, handleSelect, error, onContinue }) => {
  ...
  // Add emoji for each fitness goal (remove icon)
  const fitnessGoals = [
