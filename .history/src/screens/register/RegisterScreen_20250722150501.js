...
export default function RegisterScreen({ mode = 'register', navigation: propNavigation, route }) {
  // Lấy step từ params nếu có, mặc định là 0
...
if (!formData.firstName.trim()) {
  ...
  isValid = false;
    ...
    try {
      const register = {
        ...
        fullName: formData.firstName || "3DO",
      ...
      // Nếu user chưa có profile, chuyển sang bước đăng ký profile
      if (!dataRegister?.data?.hasProfile) {
        setCurrentStep(1); // chuyển sang step2 để đăng ký profile
        setStepHistory([0, 1]);
        showSuccessMessage("Account created! Please continue to complete your profile.");
      } else {
        await Promise.all([
  ...
  const renderStepContent = () => {
    if (mode === 'register') {
      ...
      // Sử dụng component ProfileSteps cho các bước profile (step 0-11)
      return (
        <ProfileSteps
