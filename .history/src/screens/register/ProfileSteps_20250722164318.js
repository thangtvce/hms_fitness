...
export default function ProfileSteps({
  currentStep = 0,
  formData = {},
  handleChange = (...args) => { console.log('[ProfileSteps] handleChange called with:', ...args); },
  handleToggle = (...args) => { console.log('[ProfileSteps] handleToggle called with:', ...args); },
  handleSelect = (...args) => { console.log('[ProfileSteps] handleSelect called with:', ...args); },
  setShowDatePicker = () => {},
  ...
  const handleCompleteProfile = async () => {
    ...
    const requiredFields = [
      "height", "weight", "age", "gender", "activityLevel", "dietaryPreference", "fitnessGoal", "goals", "bodyFatPercentage"
    ];
    const missingFields = requiredFields.filter((field) => !formData?.[field]);
    console.log("[COMPLETE PROFILE] formData:", formData);
    if (missingFields.length > 0) {
      showErrorFetchAPI(`Vui lòng nhập đầy đủ thông tin: ${missingFields.join(", ")}`);
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // Update giới tính và năm sinh lên user
      const userDto = {
        gender: formData?.gender || "",
        birthYear: formData?.age ? (new Date().getFullYear() - Number(formData?.age)) : undefined,
      };
      await apiUserService.updateUser(user.id, userDto);

      // Tính calorie goal
      const calorie = calculateCalorieGoal();
      setCalorieGoal(calorie);

      // Lưu các thông tin còn lại vào profile
      const profileData = {
        userId: user.id,
      ...
      console.log("[COMPLETE PROFILE] profileData gửi lên:", profileData);
      ...
      console.log("[COMPLETE PROFILE] Đã gửi API thành công!");
      ...
      showErrorFetchAPI(error?.message || error?.toString() || "Unexpected error");
    }
  }
  ...
  return (
    <View style={styles.stepContainer}>
      ...
      <View style={styles.sliderContainer}>
        <TouchableOpacity style={styles.sliderButton} onPress={() => setSelectedIndex(Math.max(0, selectedIndex - 1))}>
...
const styles = StyleSheet.create({
  ...
  // Goals, Dietary, Fitness Goal Styles
  optionsContainer: {
    width: "100%",
    marginBottom: 16,
  },
