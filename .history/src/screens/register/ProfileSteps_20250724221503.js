...
export default function ProfileSteps({
  ...
  const handleCompleteProfile = async () => {
    ...
    try {
      ...
      const birthDateStr = birthDate ? String(birthDate) : "";
      // Lấy avatar từ userData nếu có, nếu không thì để rỗng
      ...
      try {
        await apiUserService.updateUser(userData.userId, userDto);
      } catch (err) {
        setIsLoading(false);
        ...
        if (err.response && err.response.data) {
          ...
          if (data.message) {
            ...
            // errors is an object: join all values
