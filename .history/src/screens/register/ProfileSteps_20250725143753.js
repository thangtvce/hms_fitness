...
const handleStartApp = async (navigation) => {
  try {
    const userData = await AsyncStorage.getItem("userData");
      ...
      routes: [
        {
          ...
          state: {
            index: 0,
            routes: [{ name: "Home" }], // Changed from "HomeScreen" to "Home"
          },
        },
      ],
    });
  ...
  if (showRecommendations) {
    // Truyền navigation vào RecommendationsScreen để tránh lỗi scope
    return <RecommendationsScreen calorieGoal={calorieGoal} onStart={handleStartApp} formData={formData} navigation={navigation} />
  }
      ...
      <View style={styles.fitnessGoalsGrid}>
        {fitnessGoals.map((goal) => {
          ...
          return (
            ...
            >
              <View style={[styles.fitnessGoalIconContainer, selected && styles.selectedFitnessGoalIconContainer]}>
                <Text style={styles.fitnessGoalEmoji}>{goal.emoji}</Text>
              </View>
        ...
        {goalsOptions.map((goal) => (
          ...
          >
            ...
            <View style={selectedGoals.includes(goal.text) ? styles.checkedBox : styles.uncheckedBox}>
              {selectedGoals.includes(goal.text) && <Ionicons name="checkmark" size={16} color="#FFFFFF" />}
            </View>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <View style={styles.dietaryPreferencesContainer}>
        {dietaryPreferences.map((preference) => {
          ...
          return (
            ...
            >
              <Text style={styles.dietaryPreferenceEmoji}>{preference.emoji}</Text>
...
const RecommendationsScreen = ({ calorieGoal, onStart, formData, navigation: navigationProp }) => {
  // Ưu tiên dùng navigation prop nếu có, fallback sang hook nếu không
  ...
  const bmi =
    formData?.height && formData?.weight
      ? (
          ...
          Math.pow(Number.parseFloat(formData.height) / 100, 2)
        ).toFixed(1)
      : null;
