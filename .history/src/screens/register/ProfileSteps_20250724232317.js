...
export default function ProfileSteps({
    ...
    switch (step) {
      ...
      case 8: // Body Fat
        ...
        if (
          ...
          errors.bodyFatPercentage = "Please enter a valid body fat percentage (3-60%)"
        }
        break
    }
  ...
  const handleCompleteProfile = async () => {
    ...
    try {
      ...
      if (missingUserFields.length > 0) {
        setIsLoading(false)
        showErrorFetchAPI(
          `Your account is missing required information: ${missingUserFields.join(", ")}. Please update your profile first.`,
        )
        return
      }
  ...
  // Render loading and recommendations after all hooks
  if (isLoading) {
...
const AgeStep = ({ formData, handleChange, error, onContinue }) => {
  ...
  const handleContinue = () => {
    // Always update age in formData before continuing
...
const FitnessGoalStep = ({ formData, handleSelect, error, onContinue }) => {
  ...
  // Add emoji for each fitness goal
  const fitnessGoals = [
    { value: "Weight Loss", icon: "trending-down-outline", emoji: "⚖️", description: "Lose weight and burn fat" },
    { value: "Maintain", icon: "pause-outline", emoji: "🛡️", description: "Maintain current weight" },
    { value: "Muscle Gain", icon: "trending-up-outline", emoji: "💪", description: "Build muscle and gain weight" },
    { value: "Improve Endurance", icon: "pulse-outline", emoji: "🏃‍♂️", description: "Boost cardiovascular fitness" },
    { value: "Increase Strength", icon: "barbell-outline", emoji: "🏋️", description: "Get stronger and more powerful" },
    { value: "Improve Flexibility", icon: "body-outline", emoji: "🤸", description: "Enhance mobility and flexibility" },
  ]
      ...
      <View style={styles.fitnessGoalsGrid}>
        {fitnessGoals.map((goal) => {
          ...
          return (
            <TouchableOpacity
              key={goal.value}
              style={[styles.fitnessGoalCard, selected && styles.selectedFitnessGoalCard]}
              onPress={() => handleGoalSelect(goal.value)}
            >
              <View style={[styles.fitnessGoalIconContainer, selected && styles.selectedFitnessGoalIconContainer, { flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }]}> 
                <Text style={{ fontSize: 22, marginRight: 6 }}>{goal.emoji}</Text>
                <Ionicons name={goal.icon} size={22} color={selected ? "#fff" : "#0056d2"} />
              </View>
              <Text style={[styles.fitnessGoalText, selected && styles.selectedFitnessGoalText]}>{goal.value}</Text>
              <Text style={[styles.fitnessGoalDescription, selected && styles.selectedFitnessGoalDescription]}>
                {goal.description}
              </Text>
            </TouchableOpacity>
          )
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (selectedGoals.includes(goal) || selectedGoals.length < 3) {
                handleToggle("goals", goal)
              }
            }}
            disabled={!selectedGoals.includes(goal) && selectedGoals.length >= 3}
          >
            <Text style={[styles.optionText, selectedGoals.includes(goal) && styles.selectedOptionText]}>{goal}</Text>
      ...
      <View style={styles.dietaryPreferencesContainer}>
        {dietaryPreferences.map((preference) => {
          ...
          return (
            <TouchableOpacity
              key={preference}
              style={[styles.dietaryPreferenceButton, selected && styles.selectedDietaryPreferenceButton]}
              onPress={() => handlePreferenceSelect(preference)}
            >
              <Text style={[styles.dietaryPreferenceText, selected && styles.selectedDietaryPreferenceText]}>
                {preference}
              </Text>
...
const BodyFatStep = ({ formData, handleChange, error, onComplete }) => {
  ...
  const handleBodyFatChange = (text) => {
    ...
    if (cleaned && !isNaN(value) && value >= 3 && value <= 60) {
      setLocalError("");
    }
  };
 ...
 const handleComplete = () => {
  ...
  if (!bodyFat || bodyFat.trim() === "") {
    setLocalError("Please enter your body fat percentage");
    return;
  }
  if (isNaN(value) || value < 3 || value > 60) {
    setLocalError("Please enter a valid body fat percentage (3-60%)");
    return;
  }
  setLocalError("");
  onComplete();
