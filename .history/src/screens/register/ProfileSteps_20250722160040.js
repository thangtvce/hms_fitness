...
export default function ProfileSteps({
  ...
  const isProfileComplete = currentStep >= 11

  const handleStartApp = async () => {
    if (!isProfileComplete) {
      // Save current step and form data to AsyncStorage
      await AsyncStorage.setItem("profileFormData", JSON.stringify(formData))
      await AsyncStorage.setItem("profileCurrentStep", currentStep.toString())
      showErrorFetchAPI("Please complete all profile steps before continuing.")
      return
    }
  ...
  // Bọc các step trong SafeAreaView và ScrollView để tránh tràn giao diện
  const renderStep = () => {
    switch (currentStep) {
      case 0:
  ...
  // Bọc giao diện step trong SafeAreaView và ScrollView
  return (
