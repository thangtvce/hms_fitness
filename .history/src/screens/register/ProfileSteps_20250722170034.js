...
export default function ProfileSteps({
  ...
  // Real handlers to update formData
  ...
  const handleCompleteProfile = async () => {
    ...
    try {
      ...
      if (!user?.userId) {
        throw new Error("userId from AuthContext is undefined");
      }
      const user = await apiUserService.getUserById(user?.userId);
      if (!user || !user.id) {
      ...
      console.log('[COMPLETE PROFILE] updateUser gửi lên:', userDto);
