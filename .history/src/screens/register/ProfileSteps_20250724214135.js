  ...
  const handleCompleteProfile = async () => {
    ...
    try {
      ...
      const birthDate = formData?.age
        ...
        : undefined;
      // Map gender to uppercase for backend
      let genderValue = formData?.gender || "";
      if (genderValue) {
        genderValue = genderValue.toUpperCase();
        if (genderValue === "FEMALE" || genderValue === "MALE" || genderValue === "OTHER") {
          // ok
        } else {
          // fallback: if user entered something else, default to OTHER
          genderValue = "OTHER";
        }
      }
      const userDto = {
      ...
      showErrorFetchAPI(error || "An unexpected error occurred during profile creation.");
    }
  };
