      ...
      <View style={styles.modalOverlay}>
        ...
        <View style={styles.modalContentNew}>
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 90 }}>
            {/* Header Image */}
            <View style={styles.modalImageContainerNew}>
              <ExerciseImage uri={exercise.mediaUrl} name={exercise.exerciseName} style={styles.modalImageNew} />
              <View style={styles.modalImageOverlayNew}>
                <TouchableOpacity style={styles.modalCloseButtonNew} onPress={onClose}>
                  <Ionicons name="close" size={24} color="#fff" />
                </TouchableOpacity>
                {exercise.difficulty && (
                  <View style={styles.modalDifficultyBadgeNew}>
                    <Text style={styles.modalDifficultyTextNew}>{exercise.difficulty}</Text>
                  </View>
                )}
              </View>
            </View>
            {/* Content */}
            <View style={styles.modalBodyNew}>
              <Text style={styles.modalTitleNew}>{exercise.exerciseName}</Text>
              {exercise.muscleGroup && (
                <View style={styles.modalInfoRow}>
                  <Text style={styles.modalInfoLabel}>Muscle Group:</Text>
                  <Text style={styles.modalInfoValue}>{exercise.muscleGroup}</Text>
                </View>
              )}
              {exercise.description && (
                <View style={styles.modalSection}>
                  <Text style={styles.modalSectionTitle}>Description</Text>
                  <Text style={styles.modalSectionText}>{exercise.description}</Text>
                </View>
              )}
              {exercise.notes && (
                <View style={styles.modalSection}>
                  <Text style={styles.modalSectionTitle}>Notes</Text>
                  <Text style={styles.modalSectionText}>{exercise.notes}</Text>
                </View>
              )}
              {/* Stats Grid */}
              <View style={styles.modalStatsGridNew}>
                <View style={styles.modalStatCardNew}>
                  <Text style={styles.modalStatValueNew}>{exercise.sets || "N/A"}</Text>
                  <Text style={styles.modalStatLabelNew}>Sets</Text>
                </View>
                <View style={styles.modalStatCardNew}>
                  <Text style={styles.modalStatValueNew}>{exercise.reps || "N/A"}</Text>
                  <Text style={styles.modalStatLabelNew}>Reps</Text>
                </View>
                <View style={styles.modalStatCardNew}>
                  <Text style={styles.modalStatValueNew}>
                    {exercise.durationMinutes ? `${exercise.durationMinutes}m` : "N/A"}
                  </Text>
                  <Text style={styles.modalStatLabelNew}>Duration</Text>
                </View>
                <View style={styles.modalStatCardNew}>
                  <Text style={styles.modalStatValueNew}>
                    {exercise.restTimeSeconds ? `${exercise.restTimeSeconds}s` : "N/A"}
                  </Text>
                  <Text style={styles.modalStatLabelNew}>Rest</Text>
                </View>
              </View>
            </View>
          </ScrollView>
          <View style={styles.modalAddButtonContainerNew}>
            <TouchableOpacity
              style={styles.modalAddButtonNew}
              onPress={() => {
                onAddToSession(exercise)
                onClose()
              }}
            >
              <Text style={styles.modalAddButtonTextNew}>Add to Workout</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
      ...
      <View style={styles.exerciseCardHeader}>
        ...
        <View style={styles.exerciseCardInfo}>
          ...
          <View style={{ flexDirection: "row", flexWrap: "wrap", alignItems: "center", marginTop: 4 }}>
            {item.kcal && <Text style={{ fontSize: 13, color: "#666", marginRight: 10 }}>{item.kcal} kcal</Text>}
            {item.sets !== undefined && (
...
const styles = StyleSheet.create({
  ...
  // Exercise Detail Modal (NEW STYLES)
  modalOverlay: {
    flex: 1,
  ...
  modalContentNew: {
    ...
    borderTopRightRadius: 24,
    width: "100%",
    height: "85%", // Slightly taller modal
    overflow: "hidden",
  },
  modalImageContainerNew: {
    position: "relative",
    width: "100%",
  ...
  modalImageNew: {
    width: "100%",
    height: "100%",
    backgroundColor: "#E2E8F0", // Lighter placeholder background
  },
  modalImageOverlayNew: {
    position: "absolute",
    top: 0,
    left: 0,
    right: 0,
    bottom: 0,
    backgroundColor: "rgba(0,0,0,0.2)", // Lighter overlay on image
    justifyContent: "space-between",
    padding: 20,
  },
  modalCloseButtonNew: {
    alignSelf: "flex-end",
    backgroundColor: "rgba(255,255,255,0.3)", // More subtle close button background
    borderRadius: 20,
    padding: 8,
  },
  modalDifficultyBadgeNew: {
    alignSelf: "flex-start", // Align to bottom-left
  ...
  modalDifficultyTextNew: {
    fontSize: 14, // Slightly larger text
    fontWeight: "600", // Less bold
    color: "#fff",
    textTransform: "capitalize", // Capitalize first letter, not all uppercase
  },
  modalBodyNew: {
    padding: 24,
    paddingBottom: 0, // Remove bottom padding as button is fixed
  },
  modalTitleNew: {
  ...
  modalSectionTitle: {
    ...
    fontWeight: "700",
    color: "#0F172A",
    marginBottom: 10, // More space below section title
  },
  modalSectionText: {
    fontSize: 15,
    color: "#475569", // Slightly darker text for readability
    lineHeight: 22,
  ...
  modalStatsGridNew: {
    ...
    marginTop: 16, // Space from descriptions
    marginBottom: 24, // Space before button
  },
  modalStatCardNew: {
    flex: 1, // Use flex to allow cards to take available space
    minWidth: "48%", // Two cards per row
    backgroundColor: "#F1F5F9", // Lighter background for stat cards
    borderRadius: 12, // Slightly less rounded
    padding: 16,
    alignItems: "flex-start", // Align text to left
    gap: 4, // Smaller gap between value and label
  },
  modalStatValueNew: {
  ...
  modalStatLabelNew: {
    ...
    color: "#64748B",
    fontWeight: "400", // Đã thay đổi từ "500" sang "400" để nhạt hơn
    textTransform: "none", // No uppercase
  },
  modalAddButtonContainerNew: {
    position: "absolute",
    left: 0,
    right: 0,
    bottom: 0,
    ...
    borderTopWidth: 1,
    borderTopColor: "#F1F5F9", // Subtle border top
  },
  modalAddButtonNew: {
    backgroundColor: "#4F46E5", // Primary action color
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    paddingVertical: 16,
