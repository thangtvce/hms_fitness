  ...
  return (
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <View style={styles.modalOverlay}>
        ...
        <View style={[styles.modalContent, { height: '80%', width: '100%', borderRadius: 24, position: 'absolute', bottom: 0, left: 0, right: 0 }]}> 
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 90 }}>
            ...
            <View style={styles.modalBody}>
              ...
              {exercise.notes && (
                <View style={styles.modalDescription}>
                  <Text style={styles.modalDescriptionTitle}>Note</Text>
                  <Text style={styles.modalDescriptionText}>{exercise.notes}</Text>
                </View>
        )}
    ...
    navigation.setOptions({
      header: () => (
        <Header
          ...
          rightActions={[
            {
              icon: <Ionicons name="barbell" size={22} color="#fff" />,
              onPress: () => navigation.navigate("WorkoutSessionScreen"),
              style: { backgroundColor: "#0056d2", borderRadius: 8, padding: 8 },
            },
            {
              icon: <Ionicons name="filter" size={22} color="#0056d2" />,
              onPress: () => setShowFilters(true),
              style: { backgroundColor: "#fff", borderRadius: 8, padding: 8, borderWidth: 1, borderColor: "#0056d2" },
            },
          ]}
        />
      ),
    })
  }, [planId, page, pageSize, searchTerm, selectedDifficulty, selectedMuscleGroup, sortBy, planName, navigation])
      ...
      <View style={styles.content}>
        {loading ? (
          <View style={{ flex: 1, minHeight: 300, width: '100%' }}>
            <ShimmerPlaceholder style={{ flex: 1, height: '100%', width: '100%', borderRadius: 16, marginBottom: 0 }} />
          </View>
        ) : error ? (
          renderErrorState()
        ) : exercises.length === 0 ? (
