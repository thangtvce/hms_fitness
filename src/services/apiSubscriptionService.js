...
export const apiSubscriptionService = {
    ...
    // Gửi đánh giá mới cho subscription (POST)
    async postRating(ratingDto) {
        try {
            const response = await apiClient.post('/TrainerRating',ratingDto);
            // Trả về response.data.data là object rating vừa tạo
            return response.data.data;
        } catch (error) {
            throw error;
        }
    },
    // Cập nhật đánh giá đã có (PUT)
    async putRating(ratingId,ratingDto) {
        try {
            const response = await apiClient.put(`/TrainerRating/${ratingId}`,ratingDto);
            // Trả về response.data.data là object rating đã cập nhật
            return response.data.data;
        } catch (error) {
            throw error;
        }
    ...
    async cancelSubscription(subscriptionId) {
        try {
            const response = await apiClient.post(`/UserPayment/cancel/${subscriptionId}`);
            return response.data.data;
        } catch (error) {
            throw error;
        }
