...
const EditExerciseScreen = () => {
    ...
    try {
      ...
      if (response.statusCode === 200 && response.data) {
        setEditExercise({
          ...
          genderSpecific: response.data.genderSpecific || 'Unisex',
          imageUrl: response.data.imageUrl || '',
          mediaUrl: response.data.mediaUrl || '',
          caloriesBurnedPerMin: response.data.caloriesBurnedPerMin?.toString() || '',
          isPrivate: response.data.isPrivate ? 1 : 0,
        });
        setImageUri(response.data.imageUrl || null);
        setVideoUri(response.data.mediaUrl || null);
        setUseImageUpload(!response.data.imageUrl);
        setUseVideoUpload(!response.data.mediaUrl);
      } else {
  ...
  const validateExerciseForm = () => {
    ...
    if (!editExercise.exerciseName.trim()) {
      errors.exerciseName = 'Exercise name is required.';
    }
    
    if (!editExercise.categoryId) {
      errors.categoryId = 'Category is required.';
    }
    
    if (!editExercise.genderSpecific) {
      errors.genderSpecific = 'Gender specification is required.';
    }
    
    if (editExercise.imageUrl && !validateMediaUrl(editExercise.imageUrl, true)) {
      errors.imageUrl = 'Invalid image URL.';
    }
    
    if (editExercise.mediaUrl && !validateMediaUrl(editExercise.mediaUrl, false)) {
      errors.mediaUrl = 'Invalid video URL. Please use a YouTube link or a valid video URL.';
    }
    
    if (editExercise.caloriesBurnedPerMin && isNaN(parseFloat(editExercise.caloriesBurnedPerMin))) {
  ...
  const handleEditExercise = useCallback(async () => {
    ...
    try {
      setActionLoading(true);
      let imageUrl = editExercise.imageUrl;
      let mediaUrl = editExercise.mediaUrl;
      
      if (useImageUpload && imageUri && imageUri !== editExercise.imageUrl) {
        imageUrl = await uploadMedia(imageUri, true);
        if (!imageUrl) return;
      } else if (!useImageUpload && editExercise.imageUrl && !validateMediaUrl(editExercise.imageUrl, true)) {
        Alert.alert('Validation Error', 'Please enter a valid image URL.');
        return;
      }
      
      if (useVideoUpload && videoUri && videoUri !== editExercise.mediaUrl) {
        mediaUrl = await uploadMedia(videoUri, false);
        if (!mediaUrl) return;
      } else if (!useVideoUpload && editExercise.mediaUrl && !validateMediaUrl(editExercise.mediaUrl, false)) {
      ...
      const exerciseData = {
        ...
        exerciseName: editExercise.exerciseName.trim(),
        description: editExercise.description.trim(),
        categoryId: editExercise.categoryId,
        genderSpecific: editExercise.genderSpecific,
        imageUrl: imageUrl || null,
        mediaUrl: mediaUrl || null,
        caloriesBurnedPerMin: parseFloat(editExercise.caloriesBurnedPerMin) || 0,
        trainerId: user.userId,
        isPrivate: editExercise.isPrivate,
      };
  ...
  }, [editExercise, imageUri, videoUri, useImageUpload, useVideoUpload, user, resetFormAndReload]);
      ...
      <LinearGradient colors={['#4F46E5', '#6366F1', '#818CF8']} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerTextContainer}>
            ...
            <Text style={styles.headerSubtitle}>Update your exercise details</Text>
          </View>
        ...
        >
          ...
          >
            ...
            <CustomSelect
              ...
              value={editExercise.genderSpecific}
            ...
            <CustomInput
              ...
              value={editExercise.caloriesBurnedPerMin}
            ...
            <MediaUploadSection
              ...
              mediaUrl={editExercise.imageUrl}
            ...
            <MediaUploadSection
              ...
              mediaUrl={editExercise.mediaUrl}
