...
const EditWorkoutPlanScreen = () => {
    ...
    const { planId,subscriptionId,userId,trainerId } = route.params || {};
    ...
    useEffect(() => {
        const fetchWorkoutPlan = async () => {
            ...
            try {
                setLoading(true);
                const response = await trainerService.getWorkoutPlansId(planId);
                if (response.statusCode === 200 && response.data) {
                    const planData = response.data;
                    setNewPlan({
                        planId,
                        subscriptionId: subscriptionId || planData.subscriptionId,
                        userId: userId || planData.userId,
                        trainerId: trainerId || planData.trainerId,
                        ...
                        frequencyPerWeek: planData.frequencyPerWeek ? planData.frequencyPerWeek.toString() : '',
                        durationMinutes: planData.durationMinutes ? planData.durationMinutes.toString() : '',
    ...
    },[planId,subscriptionId,userId,trainerId]);

    const validateForm = () => {
        const newErrors = {};
        if (!newPlan.planId || newPlan.planId < 1) {
            newErrors.planId = 'Plan ID is required and must be a positive integer.';
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await trainerService.updateWorkoutPlan(newPlan.planId,workoutData);
            if (response.statusCode === 200) {
                showSuccessMessage('Workout plan updated successfully.');
                navigation.navigate('TrainerSubscriptionDetailScreen',{
                    subscriptionId: newPlan.subscriptionId,
                });
            } else {
                throw new Error(response.message || 'Failed to update workout plan.');
            }
        } catch (error) {
            showErrorFetchAPI(error);
        } finally {
