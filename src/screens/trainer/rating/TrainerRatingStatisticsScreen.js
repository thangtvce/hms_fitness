...
const TrainerRatingStatisticsScreen = () => {
    ...
    useEffect(() => {
        ...
        const fetchStatistics = async () => {
            try {
                ...
                const response = await trainerService.getTrainerStatistic(user.userId,params);
            ...
            <View style={styles.fieldContainer}>
                ...
                <View style={styles.statsGrid}>
                    <View style={styles.statCard}>
                        <Text style={styles.statCardLabel}>Total Ratings</Text>
                        <Text style={styles.statCardValue}>{statistics?.totalRatings || 0}</Text>
                    </View>
                    <View style={styles.statCard}>
                    ...
                    {statistics?.ratingsByTrainer?.[0] && (
                        <View style={styles.statCard}>
                            <Text style={styles.statCardLabel}>Trainer</Text>
                            <Text style={styles.statCardValue}>{statistics.ratingsByTrainer[0].trainerName}</Text>
                        </View>
                    )}
                </View>
            </View>
    ...
    const renderScoreChart = () => {
        const pieData = statistics?.ratingsByScore?.map((item) => ({
            name: `${item.score} Star${item.score > 1 ? 's' : ''}`,
            value: item.count,
            color: item.score >= 4 ? '#22C55E' : item.score >= 3 ? '#F59E0B' : '#EF4444',
        })) || [];
                ...
                <View style={styles.fieldContainer}>
                    <View style={styles.fieldHeader}>
                        <View style={styles.fieldLabelContainer}>
                            ...
                            <Text style={styles.fieldLabel}>Ratings by Score</Text>
                        </View>
                    </View>
    ...
    const renderDateChart = () => {
        const barData = {
            labels: statistics?.ratingsByCreationDate?.map((item) => `${item.month}/${item.year % 100}`) || [],
            datasets: [
                {
                    data: statistics?.ratingsByCreationDate?.map((item) => item.count) || [],
                    color: (opacity = 1) => `rgba(0, 86, 210, ${opacity})`,
                },
            ],
        };
                ...
                <View style={styles.fieldContainer}>
                    <View style={styles.fieldHeader}>
                        <View style={styles.fieldLabelContainer}>
                            ...
                            <Text style={styles.fieldLabel}>Ratings by Creation Date</Text>
                        </View>
                    </View>
                    <View style={styles.chartContainer}>
                        <BarChart
...
export default TrainerRatingStatisticsScreen;