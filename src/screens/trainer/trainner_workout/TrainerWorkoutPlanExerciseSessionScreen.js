...
const SessionEditModal = ({ visible, session, onClose, onSave, onDelete, isNew, userId }) => {
  ...
  useEffect(() => {
    if (session && !isNew) {
      setFormData({
        notes: session.notes || "",
        totalCaloriesBurned: session.totalCaloriesBurned?.toString() || "",
        totalDurationMinutes: session.totalDurationMinutes?.toString() || "",
  ...
  const handleSave = async () => {
    if (!formData.totalCaloriesBurned || !formData.totalDurationMinutes) {
      Alert.alert("Error", "Calories burned and duration are required.");
      return;
    }
    ...
    try {
      const sessionData = {
        ...
        totalCaloriesBurned: parseFloat(formData.totalCaloriesBurned) || 0,
        totalDurationMinutes: parseInt(formData.totalDurationMinutes, 10) || 0,
        startTime: formData.startTime.toISOString(),
      };

      await onSave(sessionData, isNew);
      onClose();
    } catch (error) {
      ...
      <View style={styles.modalOverlay}>
        ...
        <View style={styles.modalContent}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>{isNew ? "Create Session" : "Edit Session"}</Text>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={styles.modalBody}>
              ...
              <TextInput
                ...
                onChangeText={(text) => setFormData({ ...formData, totalCaloriesBurned: text })}
              ...
              <TextInput
                ...
                onChangeText={(text) => setFormData({ ...formData, totalDurationMinutes: text })}
              ...
              <View style={styles.modalActions}>
                {!isNew && (
                  ...
                  >
                    ...
                    <Text style={styles.removeButtonText}>Delete Session</Text>
                  </TouchableOpacity>
                )}
...
export default function TrainerWorkoutPlanExerciseSessionScreen() {
  ...
  const fetchPlanDetails = async () => {
    try {
      ...
      console.log("Plan Details Response:", JSON.stringify(response, null, 2));
    ...
    try {
      ...
      const detailedExercises = await Promise.all(
        planExercises.map(async (ex) => {
          ...
          try {
            const detail = await trainerService.getSystemFitnessExerciseById(ex.exerciseId);
  ...
  const fetchSessions = async () => {
      ...
      const response = await trainerService.getWorkoutSessionLogsByUserId(userId, { pageNumber: 1, pageSize: 50 });
  ...
  const handleSaveSession = async (sessionData, isNew) => {
    try {
      if (isNew) {
        await trainerService.createWorkoutSession(sessionData);
        Alert.alert("Success", "Session created!");
      } else {
        await trainerService.updateWorkoutSession(session.sessionId, sessionData);
  ...
  const handleDeleteSession = async (session) => {
    try {
      await trainerService.deleteWorkoutSession(session.sessionId);
  ...
  const handleViewHistory = () => {
    navigation.navigate("WorkoutHistoryScreen", { userId });
  };
  ...
  const renderExerciseItem = ({ item, index }) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate("TrainerWorkoutPlanExercisesScreen", { planId, planName, userId })}
      activeOpacity={0.8}
    >
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.errorContainer}>
          ...
          <TouchableOpacity style={styles.retryButton} onPress={() => { fetchPlanDetails(); fetchExercises(); fetchSessions(); }}>
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollContainer}>
            ...
            <SafeAreaView style={styles.heroContent}>
              <View style={styles.heroHeader}>
                ...
                <View style={styles.heroActions}>
                  ...
                  <TouchableOpacity style={styles.heroButton} onPress={handleViewHistory}>
        ...
        <View style={styles.contentSection}>
          <Text style={styles.dayTitle}>{planDetails?.planName || "Client Workout Plan"}</Text>

          <View style={styles.planInfo}>
            <Text style={styles.planInfoText}>User: {planDetails?.userFullName || "N/A"}</Text>
            <Text style={styles.planInfoText}>Start: {planDetails?.startDate ? new Date(planDetails.startDate).toLocaleDateString("en-US") : "N/A"}</Text>
            <Text style={styles.planInfoText}>End: {planDetails?.endDate ? new Date(planDetails.endDate).toLocaleDateString("en-US") : "N/A"}</Text>
            <Text style={styles.planInfoText}>Frequency: {planDetails?.frequencyPerWeek} times/week</Text>
            <Text style={styles.planInfoText}>Status: {planDetails?.status || "N/A"}</Text>
            {planDetails?.description && (
              <Text style={styles.planInfoText}>Description: {planDetails.description}</Text>
            )}
          </View>
      ...
      <SafeAreaView style={styles.bottomSection}>
        ...
        >
          ...
          <Text style={styles.settingsText}>Manage Exercises</Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={[styles.startButton, !exercises.length && styles.startButtonDisabled]}
          onPress={() => {
            setSelectedSession(null);
            setIsNewSession(true);
            setModalVisible(true);
