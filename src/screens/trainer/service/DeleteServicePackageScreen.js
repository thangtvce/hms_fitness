...
const DeleteServicePackageScreen = () => {
  ...
  const handleDeletePackage = async () => {
    try {
      ...
      if (response.statusCode === 200) {
        ...
        throw new Error(`Error ${response.statusCode}: ${response.message || 'Failed to delete package.'}`);
      }
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Delete Package</Text>
      </View>
      <View style={styles.content}>
        <View style={styles.iconContainer}>
          <Ionicons name="trash-outline" size={32} color="#EF4444" />
        </View>
        <Text style={styles.title}>Delete Service Package</Text>
        <Text style={styles.message}>Are you sure you want to delete this package?</Text>
        <View style={styles.buttonContainer}>
          <TouchableOpacity style={styles.cancelButton} onPress={() => navigation.goBack()}>
            <Text style={styles.cancelButtonText}>Cancel</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.confirmButton} onPress={handleDeletePackage} disabled={actionLoading}>
...
export default DeleteServicePackageScreen;