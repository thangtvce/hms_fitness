...
const AddExerciseToPlanScreen = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const planResponse = await trainerService.getWorkoutPlansById(planId);
        ...
        const myResponse = await trainerService.getExerciseByTrainerId(myParams);
        const bankResponse = await trainerService.getExerciseByBank(bankParams);
  ...
  const validateForm = () => {
    ...
    if (!planId || planId < 1) {
      newErrors.planId = 'PlanId is required and must be a positive integer.';
    }

    if (!exerciseId || exerciseId < 1) {
      newErrors.exerciseId = 'ExerciseId is required and must be a positive integer.';
    }

    if (sets === null || sets === undefined || sets === '' || isNaN(sets) || parseInt(sets) < 1) {
      newErrors.sets = 'Sets is required and must be a positive integer.';
    }

    if (reps === null || reps === undefined || reps === '' || isNaN(reps) || parseInt(reps) < 1) {
      newErrors.reps = 'Reps is required and must be a positive integer.';
    }

    if (durationMinutes === null || durationMinutes === undefined || durationMinutes === '' || isNaN(durationMinutes) || parseInt(durationMinutes) < 1) {
      newErrors.durationMinutes = 'DurationMinutes is required and must be a positive integer.';
    }

    if (restTimeSeconds === null || restTimeSeconds === undefined || restTimeSeconds === '' || isNaN(restTimeSeconds) || parseInt(restTimeSeconds) < 0) {
      newErrors.restTimeSeconds = 'RestTimeSeconds is required and must be a non-negative integer.';
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = {
        ...
        restTimeSeconds: restTimeSeconds ? parseInt(restTimeSeconds) : null,
        notes: notes,
      };
      const response = await trainerService.createPlanExercise(data);
...
export default AddExerciseToPlanScreen;