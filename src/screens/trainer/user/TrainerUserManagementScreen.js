...
const TrainerUserManagementScreen = () => {
    ...
    const fetchAllUsers = async () => {
        try {
            ...
            const response = await trainerService.getAllUserForTrainer(params);
            if (response.statusCode === 200 && Array.isArray(response.data?.subscribers)) {
                let fetchedUsers = response.data.subscribers;
                const timestamp = Date.now();
                const preparedUsers = fetchedUsers.map((item,index) => ({
                    ...item,
                    _key: item.subscriptionId ? `sub-${item.subscriptionId}` : `sub-unknown-${timestamp}-${index}`,
                }));

                let updatedAllUsers = pageNumber === 1 ? preparedUsers : [...allUsers,...preparedUsers];
                ...
                updatedAllUsers = uniqueById(updatedAllUsers);

                setAllUsers(updatedAllUsers);
                setDisplayedUsers(updatedAllUsers);

                const currentTotalItems = response.data.totalCount || updatedAllUsers.length;
    ...
    const UserItem = ({ item }) => {
        ...
        const handleUserPress = () => {
            navigation.navigate('TraineeDetailScreen',{
                userId: item.userId,
        ...
        return (
            ...
            >
                <View style={styles.cardContainer}>
                    <TouchableOpacity activeOpacity={0.9} onPress={handleUserPress} style={styles.cardContent}>
                        ...
                        <View style={styles.descriptionSection}>
                            ...
                            <Text style={styles.transactionText} numberOfLines={1}>
                                Price: ${item.packagePrice?.toLocaleString() || '0'}
                            </Text>
                    ...
                    <TouchableOpacity style={styles.addWorkoutPlanButton} onPress={handleSubscriptionPress}>
                        <Ionicons name="document-text-outline" size={24} color="#0056D2" />
                    </TouchableOpacity>
                </View>
            </Animated.View>
        );
...
export default TrainerUserManagementScreen;