...
const TrainerSubscriptionStatisticsScreen = () => {
    ...
    useEffect(() => {
        ...
        const fetchStatistics = async () => {
            try {
                ...
                const response = await trainerService.getSubscriptionStatistic(params);
        ...
        >
            ...
            {(filters.startDate || filters.endDate) && (
                <TouchableOpacity
                    ...
                    onPress={() => setFilters({ startDate: null,endDate: null })}
                >
            ...
            <View style={styles.fieldContainer}>
                ...
                <View style={styles.statsGrid}>
                    <View style={styles.statCard}>
                        <Text style={styles.statCardLabel}>Total Subscriptions</Text>
                        <Text style={styles.statCardValue}>{statistics?.totalSubscriptions || 0}</Text>
                    </View>
                    <View style={styles.statCard}>
                    ...
                    {statistics?.mostPopularPackage && (
                        <View style={styles.statCard}>
                            <Text style={styles.statCardLabel}>Most Popular Package</Text>
                            <Text style={styles.statCardValue}>{statistics.mostPopularPackage.packageName}</Text>
                        </View>
                    )}
                </View>
            </View>
    ...
    const renderStatusChart = () => {
        const pieData = statistics?.subscriptionsByStatus?.map((item) => ({
            name: item.status.charAt(0).toUpperCase() + item.status.slice(1),
            value: item.count,
            color:
                item.status === 'paid' || item.status === 'systempaid' || item.status === 'active'
                    ? '#22C55E'
                    : item.status === 'pending'
                        ? '#F59E0B'
                        : '#EF4444',
                ...
                <View style={styles.fieldContainer}>
                    <View style={styles.fieldHeader}>
                        <View style={styles.fieldLabelContainer}>
                            ...
                            <Text style={styles.fieldLabel}>Subscriptions by Status</Text>
                        </View>
                    </View>
    ...
    const renderDateChart = () => {
        const barData = {
            labels: statistics?.subscriptionsByCreationDate?.map((item) => `${item.month}/${item.year % 100}`) || [],
            datasets: [
                {
                    data: statistics?.subscriptionsByCreationDate?.map((item) => item.count) || [],
                    color: (opacity = 1) => `rgba(0, 86, 210, ${opacity})`,
                },
            ],
        };
                ...
                <View style={styles.fieldContainer}>
                    <View style={styles.fieldHeader}>
                        <View style={styles.fieldLabelContainer}>
                            ...
                            <Text style={styles.fieldLabel}>Subscriptions by Creation Date</Text>
                        </View>
                    </View>
                    <View style={styles.chartContainer}>
                        <BarChart
                ...
                rightActions={[
                    {
                        ...
                        color: "#0056D2",
                        showBadge: filters.startDate || filters.endDate,
...
export default TrainerSubscriptionStatisticsScreen;