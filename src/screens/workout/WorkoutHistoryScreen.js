...
const WorkoutHistoryScreen = () => {
    ...
    try {
      if (activity && activity.exerciseId) {
        ...
        if (exercise) {
          ...
          showErrorMessage("Could not fetch exercise details.");
        }
      } else {
        showErrorMessage("Cannot open details: activity is invalid. See console for details.")
      }
    } catch (err) {
    ...
    return (
      <View style={styles.statsContainer}>
        <View style={styles.statsRow}>
          ...
          <View style={styles.statCard}>
            <Text style={styles.statValue}>{Math.round(stats.totalDuration)}</Text>
        ...
        <View style={styles.avgStatsCard}>
          ...
          <View style={styles.avgStatsRow}>
            ...
            <View style={styles.avgStat}>
              <Text style={styles.avgStatValue}>{Math.round(stats.avgDurationPerDay)}</Text>
              <Text style={styles.avgStatLabel}>seconds</Text>
            </View>
          </View>
        </View>
      </View>
  ...
  const renderChart = () => {
    ...
    historySessions.forEach((session) => {
      ...
      dailyData[dateKey].calories += session.totalCaloriesBurned
      dailyData[dateKey].duration += session.totalDurationMinutes

      const activities = Array.isArray(session.userActivities) ? session.userActivities : []
      dailyData[dateKey].exercises += activities.length
    })

    // Get last 7 days of data
    const sortedDates = Object.keys(dailyData).sort((a,b) => {
      const dateA = dailyData[a].date
      const dateB = dailyData[b].date
    ...
    const caloriesData = {
      ...
      datasets: [
        {
          data: last7Days.map((dateKey) => dailyData[dateKey].calories),
    ...
    const exerciseData = {
      ...
      datasets: [
        {
          data: last7Days.map((dateKey) => dailyData[dateKey].exercises),
    ...
    const durationData = {
      ...
      datasets: [
        {
          data: last7Days.map((dateKey) => Math.round(dailyData[dateKey].duration)),
    ...
    // Show each chart vertically, one per row
    return (
      <View style={styles.chartsContainer}>
        ...
        <View style={styles.chartContainer}>
          <Text style={styles.chartTitle}>Workout Duration (Seconds)</Text>
          <LineChart
            data={durationData}
            width={width - 40}
            height={160}
        ...
        <TouchableOpacity onPress={() => toggleSession(item.sessionId)} style={styles.sessionHeader}>
          ...
          <View style={styles.sessionInfo}>
            ...
            <View style={styles.sessionStats}>
              ...
              <Text style={styles.sessionStat}>{Math.round(item.totalDurationMinutes)} sec</Text>
              <Text style={styles.sessionStat}>•</Text>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={[styles.tabContainer,{ marginTop: 80 }]}>
