...
const ExerciseImage = ({ uri, name, style }) => {
  ...
  const fallbackUrl = `https://source.unsplash.com/400x300/?fitness,${encodeURIComponent(name || "workout")}`
    ...
    >
      ...
      <View style={styles.exerciseCardHeader}>
        ...
        <View style={styles.exerciseCardInfo}>
          <View style={styles.exerciseCardTitleRow}>
            ...
            <TouchableOpacity style={styles.exerciseCardFavorite} onPress={() => setSelectedExercise(item)}>
          ...
          {item.difficulty && (
            <View
              style={[
                ...
                {
                  backgroundColor: DIFFICULTY_OPTIONS.find((opt) => opt.value === item.difficulty)?.color || "#64748B",
                },
              ]}
            >
              <Text style={styles.exerciseCardDifficultyText}>{item.difficulty}</Text>
            </View>
          )}

          {item.muscleGroup && (
            <View style={styles.exerciseCardMuscleGroup}>
              <Ionicons name="body" size={14} color="#4F46E5" />
              <Text style={styles.exerciseCardMuscleGroupText}>{item.muscleGroup}</Text>
            </View>
          )}
        </View>
      </View>
      ...
      <View style={styles.exerciseCardStats}>
        <View style={styles.exerciseCardStat}>
          ...
          <Text style={styles.exerciseCardStatText}>{item.sets || "N/A"} sets</Text>
        </View>
        <View style={styles.exerciseCardStat}>
          <Ionicons name="fitness" size={16} color="#3B82F6" />
          <Text style={styles.exerciseCardStatText}>{item.reps || "N/A"} reps</Text>
        </View>
        <View style={styles.exerciseCardStat}>
          <Ionicons name="time" size={16} color="#F59E0B" />
          <Text style={styles.exerciseCardStatText}>{item.durationMinutes ? `${item.durationMinutes}m` : "N/A"}</Text>
        </View>
      </View>

      {/* Add Button */}
      <TouchableOpacity
        style={styles.exerciseCardAddButton}
        onPress={(e) => {
          e.stopPropagation()
          addToSession(item)
        }}
      >
        ...
        <Text style={styles.exerciseCardAddButtonText}>Add to Workout</Text>
      </TouchableOpacity>
    </TouchableOpacity>
  )
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerSubtitle}>Choose your exercises</Text>
        </View>
        <TouchableOpacity
          style={[styles.headerButton, sessionCount > 0 && styles.headerButtonActive]}
