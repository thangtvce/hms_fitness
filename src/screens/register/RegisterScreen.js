...
export default function RegisterScreen({ navigation: propNavigation,route }) {
    ...
    const loadSavedData = async () => {
      try {
        ...
        if (savedData) {
          ...
          // Validate birthDate to ensure it's in yyyy-mm-dd format
          if (parsedData.birthDate) {
            const regex = /^\d{4}-\d{2}-\d{2}$/;
            const [year,month,day] = parsedData.birthDate.split("-").map(Number);
            const date = new Date(year,month - 1,day);
            if (
              !regex.test(parsedData.birthDate) ||
              isNaN(date.getTime()) ||
              date.getFullYear() !== year ||
              date.getMonth() + 1 !== month ||
              date.getDate() !== day
            ) {
              parsedData.birthDate = ""; // Reset invalid birthDate
            }
          }
  ...
  const validateCurrentStep = () => {
    ...
    if (currentStep === 0) {
      if (!formData.firstName.trim()) {
        newErrors.firstName = "Your name is required to personalize your account";
        isValid = false;
      ...
      if (!formData.password) {
        ...
        newErrors.password = "Password must be at least 8 characters for better security";
        isValid = false;
      ...
      if (!formData.confirmPassword) {
        ...
        newErrors.confirmPassword = "Passwords don't match. Please check and try again";
        isValid = false;
      }
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (error.response?.data?.errors) {
        ...
        if (serverErrors.Gender) newErrors.gender = serverErrors.Gender[0];
        if (serverErrors.BirthDate) newErrors.birthDate = serverErrors.BirthDate[0];
        setErrors(newErrors);
    ...
    return (
      <View style={styles.stepIndicatorsContainer}>
        {Array.from({ length: totalSteps }).map((_,index) => (
          <View
            key={index}
            style={[styles.stepIndicator,index <= currentStep ? styles.activeStepIndicator : {}]}
          />
        ))}
      </View>
    );
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 0:
        return <NameStep formData={formData} handleChange={handleChange} errors={errors} setErrors={setErrors} />;
      case 1:
        return (
          <AccountSetupStep
            formData={formData}
...
const NameStep = ({ formData,handleChange,errors,setErrors }) => {
  ...
  const handleDateChange = (value) => {
    ...
    if (formatted.length === 10 && !validateDate(formatted)) {
      setErrors((prev) => ({
        ...prev,
        birthDate: "Please enter a valid birth date (yyyy-mm-dd)",
      }));
  ...
  return (
    <View style={styles.stepContainer}>
      ...
      <Text style={styles.stepDescription}>Let's start by getting to know you. Please provide your personal details.</Text>
      ...
      <View style={styles.inputContainer}>
        ...
        {errors.firstName ? (
          <Text style={styles.errorText}>
            <Ionicons name="alert-circle-outline" size={14} color="#EF4444" /> {errors.firstName}
          </Text>
        ) : null}
      </View>
        ...
        <View style={styles.genderContainer}>
          {["Male","Female","Other"].map((option) => (
            <TouchableOpacity
              ...
              style={[
                styles.genderButton,
                formData.gender === option ? styles.genderButtonSelected : null,
            ...
            >
              <Text
                style={[
                  ...
                  formData.gender === option ? styles.genderButtonTextSelected : null,
                ]}
              >
                {option}
              </Text>
      ...
      <View style={styles.inputContainer}>
        ...
        <View style={[styles.iconInputContainer,errors.birthDate ? styles.inputError : null]}>
      ...
      <Text style={styles.tipText}>
        Enter your birth date in yyyy-mm-dd format (e.g., 2000-01-01). This helps us personalize your experience.
      </Text>
    </View>
  );
};
