...
const ExportHistoryScreen = ({ route, logs, navigation }) => {
  ...
  const handleSaveImage = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Nutrition report has been saved to photo library.')
    } catch (e) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f5f5f5', padding: 0 }}>
      <Header
        title="Export Nutrition Report"
        onBack={() => navigation.goBack()}
        rightActions={[]}
      />
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16, marginTop: 90 }}>
