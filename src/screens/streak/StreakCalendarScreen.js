...
export default function StreakCalendarScreen() {
  ...
  // Mark the lastCheckInDate if it exists
  if (lastCheckInDate) {
    const lastCheckInDateStr = lastCheckInDate.slice(0, 10)
    markedDates[lastCheckInDateStr] = {
      selected: true,
      customStyles: {
        container: styles.markedDayContainer,
        text: styles.markedDayText,
      },
    }

    // Add flame emoji if there's an active streak
    if (currentStreak > 0) {
      markedDates[lastCheckInDateStr].emoji = "🔥"
      if (lastCheckInDateStr === today) {
        markedDates[lastCheckInDateStr].customStyles = {
          ...markedDates[lastCheckInDateStr].customStyles,
        ...
        <>
          ...
          <View style={styles.currentStreakContainer}>
            {currentStreak > 0 && (
              <Ionicons
                name="flame"
                size={flameStyle.iconSize}
                color={flameStyle.iconColor}
                style={[styles.flameIcon, flameStyle.iconShadow]}
              />
            )}
