...
const EditPostScreen = ({ route,navigation }) => {
  ...
  const [contentLength,setContentLength] = useState(post.content?.replace(/<[^>]*>/g,"").length || 0);
      ...
      <script>
        ClassicEditor
          .create( document.querySelector( '#editor' ), {
            ...
            language: 'en',
            initialData: '${(post.content || "").replace(/'/g,"\\'")}'
          } )
          .then( editor => {
  ...
  const handleTakePhoto = async () => {
    try {
      ...
      showErrorMessage("Failed to take photo.");
    } finally {
      setImageUploading(false);
      setShowImageOptions(false);
    }
  ...
  const handleSubmit = async () => {
    ...
    let finalImageUrl = cloudImageUrl || thumbnail;
    ...
    try {
      ...
      showSuccessMessage("Your post has been updated successfully!");
      setTimeout(() => {
        navigation.goBack();
      },1000);
    } catch (error) {
  ...
  if (!post || !post.postId) {
    showErrorMessage("Invalid post data. Please try again.");
    setTimeout(() => {
      navigation.goBack();
    },1000);
    return null;
