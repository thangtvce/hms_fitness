...
const CreatePostScreen = ({ route, navigation }) => {
  ...
  const DEFAULT_IMAGE = "DEFAULT_IMAGE"; // Replace with a valid URL if needed

  // Animation and initial data effects
  useEffect(() => {
    Animated.parallel([
  ...
  const validateThumbnail = (thumb) => {
    ...
    // If it's a base64 string, skip length validation (will be uploaded and replaced by URL)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (thumbnail && thumbnail.startsWith("data:image") && !cloudImageUrl) {
        ...
        // Debug: log the base64 string length before upload
        const uploadResult = await apiUploadImageCloudService.uploadImage(formData);
        // Debug: log the returned imageUrl after upload
  ...
  const canPublish = content.trim().length > 0 && content.trim().length <= 500 && !loading;
      ...
      >
        ...
        >
          <View style={styles.modalOverlay}>
            <Animated.View
              style={[styles.imageOptionsModal, { transform: [{ scale: scaleAnim }] }]}
            >
        ...
        >
          <View style={styles.modalOverlay}>
            <Animated.View
              style={[styles.urlInputModal, { transform: [{ scale: scaleAnim }] }]}
            >
              <View style={styles.modalHeader}>
                <Text style={styles.modalTitle}>Enter Image URL</Text>
                <TouchableOpacity onPress={cancelUrlInput}>
                  <Ionicons name="close" size={24} color="#64748B" />
                </TouchableOpacity>
              </View>
              <TextInput
              ...
              <View style={styles.urlModalButtons}>
                <TouchableOpacity
                  style={[styles.urlModalButton, styles.cancelButton]}
                  onPress={cancelUrlInput}
                >
                  <Text style={styles.urlModalButtonText}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.urlModalButton, styles.confirmButton]}
                ...
                >
                  {imageUploading ? (
                    ...
                    <Text style={[styles.urlModalButtonText, { color: "#FFFFFF" }]}>
                      Confirm
                    </Text>
                  )}
                </TouchableOpacity>
        ...
        >
          <View style={styles.modalOverlay}>
            <Animated.View
              style={[styles.tagsModal, { transform: [{ scale: scaleAnim }] }]}
            >
              <View style={styles.modalHeader}>
